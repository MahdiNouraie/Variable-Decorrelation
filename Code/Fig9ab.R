# This script reproduces Figure 9(a) and 9(b) of the paper
...
library(cmna) # Load the cmna package for AirHOLP
...
# Stability Selection with SCAD penalty
...
n <- 50  # Number of samples
p <- 500  # Number of predictors
# Stability Selection with SCAD penalty and pre-processing
...
for (i in 1:100){
  ...
  x_ranked <- x[, column_order] # Reorder columns of x based on the rank
  ...
  colnames(x) <- names # Set column names of x to original names
...
# Remove unnecessary objects and run the garbage collector
rm(AHOLP, cv_fit, gram, ranked_features, SCAD_fit, x_ranked, data, lasso_model, S, S_list, 
   S_stable_1sd, stability_results, column_order,
   stab_values, x, y, names, Threshold,
   B, candidate_set, col_means, S_stable_1sd_filtered,
   x_sub, i, index_of_stable_1sd, lambda, lambda_idx,
   max_stability, y_sub, temp, seed, stability_1sd_threshold,
   sample_index, significant_predictors,
...
# Plot stability comparison across methods
...
# Plot F1-score comparison across methods and thresholds
