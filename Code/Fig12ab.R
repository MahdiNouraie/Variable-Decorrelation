# This script reproduces Figure 12(a) and 12(b) of the paper
...
library(cmna) # Load the cmna package
...
#  [1] ggpubfigs_0.0.1 cmna_1.0.5      reshape2_1.4.4  ggplot2_3.5.2  
#[5] glmnet_4.1-8    Matrix_1.7-1    MASS_7.3-64    

#loaded via a namespace (and not attached):
#  [1] generics_0.1.3    shape_1.4.6.1     stringi_1.8.4     lattice_0.22-6   
#[5] digest_0.6.37     magrittr_2.0.3    grid_4.4.2        iterators_1.0.14 
#[9] pkgload_1.4.0     fastmap_1.2.0     foreach_1.5.2     plyr_1.8.9       
#[13] processx_3.8.4    sessioninfo_1.2.3 pkgbuild_1.4.6    survival_3.8-3   
#[17] ps_1.8.1          urlchecker_1.0.1  promises_1.3.2    purrr_1.0.2      
#[21] scales_1.4.0      codetools_0.2-20  cli_3.6.5         shiny_1.10.0     
#[25] rlang_1.1.6       ellipsis_0.3.2    munsell_0.5.1     splines_4.4.2    
#[29] withr_3.0.2       remotes_2.5.0     cachem_1.1.0      devtools_2.4.5   
#[33] tools_4.4.2       memoise_2.0.1     dplyr_1.1.4       colorspace_2.1-1 
#[37] httpuv_1.6.15     curl_6.0.1        vctrs_0.6.5       R6_2.6.1         
#[41] mime_0.12         lifecycle_1.0.4   stringr_1.5.1     htmlwidgets_1.6.4
#[45] fs_1.6.5          usethis_3.1.0     miniUI_0.1.1.1    callr_3.7.6      
#[49] desc_1.4.3        pkgconfig_2.0.3   pillar_1.10.2     later_1.4.1      
#[53] gtable_0.3.6      glue_1.8.0        profvis_0.4.0     Rcpp_1.0.14      
#[57] tibble_3.2.1      tidyselect_1.2.1  rstudioapi_0.17.1 xtable_1.8-4     
...
generate_data <- function(n, p) {
  ...
  active_indices <- seq(1, p, by = group_size)  # First element of each group
...
for (i in 1:100){
  ...
  colnames(x) <- paste0("V", 1:ncol(x)) # Column names for predictors
  ...
  important_vars <- paste0("V", seq(1, p, by = p / 5))  # First element of each group
...
# Stability Selection with Lasso + AirHOLP and Grahm-Schmidt
set.seed(26) # Set seed for reproducibility
n <- 50  # Number of samples
p <- 1000  # Number of predictors
...
for (i in 1:100){
  ...
  colnames(x) <- names # Set column names of orthogonalized matrix
  cv_lasso <- cv.glmnet(x, y, nfolds = 10, alpha = 1) # Fit Lasso with 10-fold CV
...
rm(cv_lasso, data, lasso_model, S, S_list, 
   ...
   Threshold, AHOLP, gram, ranked_features, x_ranked,
   column_order, names); gc() 
...
tmp_df$Method[tmp_df$Method == "S1"] <- "Without" # Rename method
tmp_df$Method[tmp_df$Method == "S2"] <- "With" # Rename method
...
metrics_results1 <- calculate_F1(results, c(6, 62)) # F1 score for threshold 0.6
metrics_results2 <- calculate_F1(results, c(7, 72)) # F1 score for threshold 0.7
metrics_results3 <- calculate_F1(results, c(8, 82)) # F1 score for threshold 0.8
metrics_results4 <- calculate_F1(results, c(9, 92)) # F1 score for threshold 0.9
metrics_results <- rbind(metrics_results1, metrics_results2, 
                         metrics_results3, metrics_results4) # Combine results
...
metrics_results$Method <- as.character(metrics_results$Method) # Convert method to character
metrics_results$Method[metrics_results$Method == 1] <- 'Without' # Rename method
metrics_results$Method[metrics_results$Method == 2] <- 'With' # Rename method

metrics_results$Method <- as.factor(metrics_results$Method) # Convert method to factor
metrics_results$Threshold <- as.factor(metrics_results$Threshold) # Convert threshold to factor
