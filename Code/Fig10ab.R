# This script reproduces Figure 10(a) and 10(b) of the paper
...
library(ncvreg) # Load the ncvreg package
...
#  [1] ggpubfigs_0.0.1 cmna_1.0.5      ncvreg_3.15.0   reshape2_1.4.4 
...
# Stability selection with MCP penalty
...
for (i in 1:100){
  ...
  candidate_set <- cv_fit$lambda  # Candidate set of lambda values
...
rm(cv_fit, MCP_fit, data, S, S_list, 
...
n <- 50  # Number of samples
p <- 500  # Number of predictors
# Stability selection with MCP penalty and pre-processing
...
for (i in 1:100){
  ...
  names <- colnames(x_ranked) # Save the names of the predictors
...
# Remove unnecessary objects and run the garbage collector
rm(AHOLP, cv_fit, gram, ranked_features, MCP_fit, x_ranked, data, 
   S, S_list, S_stable_1sd, stability_results, column_order,
   stab_values, x, y, names, Threshold,j, k,
   B, candidate_set, col_means, S_stable_1sd_filtered,
   x_sub, i, index_of_stable_1sd, lambda, lambda_idx,
   max_stability, y_sub, temp, seed, stability_1sd_threshold,
   sample_index, significant_predictors,
...
tmp_df <- melt(tmp_df) # Melt the data frame to long format
...
# Convert Method to character and rename
